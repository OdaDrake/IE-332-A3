...
// View mode for top/bottom companies
$fhViewMode = isset($_GET['fh_view']) ? $_GET['fh_view'] : 'top';
$fhLimit = 10; // Changed from showing all to top/bottom 10
...
if ($fhStart !== '' && $fhEnd !== '') {
    $orderDirection = ($fhViewMode === 'bottom') ? 'ASC' : 'DESC';
    ...
    $fhCompanySql = "
        ...
        ORDER BY AvgHealthScore $orderDirection
        LIMIT $fhLimit
    ";
...
// Event selector with search capability
$deEventID = isset($_GET['de_event']) ? (int)$_GET['de_event'] : 0;
$deSearch = isset($_GET['de_search']) ? trim($_GET['de_search']) : '';

// Get list of disruption events (filtered by date range and search)
...
if ($deSearch !== '') {
    $deSearchEsc = $conn->real_escape_string($deSearch);
    $eventListSql .= " AND (dc.CategoryName LIKE '%$deSearchEsc%' OR de.EventID LIKE '%$deSearchEsc%')";
}

$eventListSql .= " ORDER BY de.EventDate DESC LIMIT 100";
...
<div class="page">
    ...
    <div class="card">
        ...
        <form method="get" class="df-form">
            ...
            <input type="hidden" name="fh_view" value="<?php echo htmlspecialchars($fhViewMode); ?>">
        ...
        <div class="view-toggle">
            <form method="get" style="display: inline;">
                ...
                <input type="hidden" name="fh_view" value="top">
                <button type="submit" class="<?php echo ($fhViewMode === 'top') ? 'active' : ''; ?>">
                    Top 10
                </button>
            </form>
            <form method="get" style="display: inline;">
                ...
                <input type="hidden" name="fh_start" value="<?php echo htmlspecialchars($fhStart); ?>">
                <input type="hidden" name="fh_end" value="<?php echo htmlspecialchars($fhEnd); ?>">
                <input type="hidden" name="fh_view" value="bottom">
                <button type="submit" class="<?php echo ($fhViewMode === 'bottom') ? 'active' : ''; ?>">
                    Bottom 10
                </button>
            </form>
        </div>
        ...
        <div class="chart-header">
            <h3 class="chart-title">
                <?php echo ($fhViewMode === 'bottom') ? 'Bottom 10' : 'Top 10'; ?> Companies by Average Financial Health
            </h3>
            <p class="chart-subtitle">
                <?php echo ($fhViewMode === 'bottom') ? 'Lowest' : 'Highest'; ?> health scores in the selected period.
            </p>
        </div>
    ...
    <div class="card">
        <div class="module-header">
            ...
            <p class="subtitle">
                Select a disruption event to see all affected companies and impact levels. Events are filtered by the date range above.
            </p>
        </div>
        ...
        <form method="get" class="df-form">
            ...
            <div class="df-field" style="min-width: 200px;">
                <label for="de_search">Search by Category or ID</label>
                <input type="text" id="de_search" name="de_search" 
                       placeholder="e.g., Storm, 123"
                       value="<?php echo htmlspecialchars($deSearch); ?>">
            </div>
        ...
        <?php if (count($eventListOptions) === 0 && $deSearch !== ''): ?>
            <div class="no-data" style="margin-top: 12px;">
                No events found matching "<?php echo htmlspecialchars($deSearch); ?>" in the selected date range.
            </div>
        <?php endif; ?>
        ...
        <?php else: ?>
            <div class="no-data">
                Please select a disruption event to view affected companies. Use the search box to find specific events.
            </div>
        <?php endif; ?>
    </div>
    ...
    <div class="card">
        ...
        <form method="get" class="df-form">
            ...
            <input type="hidden" name="de_search" value="<?php echo htmlspecialchars($deSearch); ?>">
